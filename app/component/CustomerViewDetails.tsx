...
const SidebarUserUpdateForm: React.FC<CustomerViewDetailsProps> = ({
  ...
  const fetchData = async () => {
    if (
      ...
      console.log("+++++++++", customer.face_id_url);
      ...
      try {
        ...
        console.error("Error occurred while fetching face image:", error);
        toast.error("Failed to get Image");
      }
    }
  };
  ...
  // useEffect(() => {
  //   // if (customer || hitApi) {
  //   console.log("customer use effect run");
  //   fetchData();
  //   fetchIdCard();
  //   fetchDataVideo();
  //   fetchUserSignature();
        ...
        if (customer && customer.face_id_url !== undefined) {
          try {
            ...
            toast.success(
              `${verification
                .replaceAll("_", " ")
                .replace(/^./, (c) => c.toUpperCase())} is rejected`
            );
            setHitApi(!hitApi);
        ...
        if (customer && customer.face_id_url !== undefined) {
          ...
          try {
            ...
            toast.success(
              `${verification
                .replaceAll("_", " ")
                .replace(/^./, (c) => c.toUpperCase())} is approved`
            );
            // toast.success("Successfully get");
            setHitApi(!hitApi);
            ...
            <div className="px-0 py-0 bg-white rounded-xl flex-col justify-start items-start gap-5 inline-flex w-full">
              ...
              <div className=" flex flex-col gap-y-3 md:flex-row md:justify-between w-full">
                <button
                  ...
                  className="bg-approveBtn hover:bg-primary-600 text-white w-full md:w-[49%] p-3 rounded"
                >
                  Approve
                </button>
                <button
                  onClick={() => reject("liveness_detection")}
                  className="bg-rejectBtn hover:!bg-[#d33] text-white w-full md:w-[49%] p-3 rounded"
                >
                  Reject
                </button>
              </div>
